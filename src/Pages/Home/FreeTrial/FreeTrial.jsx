...
const FreeTrial = () => {
  ...
  const handleFreeTrialApplication = (e) => {
    ...
    axios
      .post("http://localhost:3000/freeTrialUsers", FreeTrialApplyData)
      .then((res) => {
      ...
      <section className="flex lg:flex-row flex-col items-center justify-between lg:bg-slate-100 rounded-md dark:bg-black lg:px-[15%]  lg:h-[600px]">
        ...
        <div className=" h-screen flex items-center justify-center">
          <div className="max-w-md p-8 dark:bg-[#191919] bg-white shadow-md rounded-md">
            <h1 className="lg:text-4xl text-3xl font-bold text-blue-500 mb-6">
            ...
            <form onSubmit={handleFreeTrialApplication} className="mb-4 ">
              <div>
                <select
                  name="Domain"
                  defaultValue=""
                  className="select p-0 appearance-none w-full border-none outline-none text-black text-sm font-semibold"
                >
                  <option disabled value="">
                    Select Domain
                  </option>
                  {domain.map((dom) => (
                    <option key={dom._id} value={dom.name}>
                      {dom.name}
                    </option>
                  ))}
                  <option disabled value="">
              ...
              <p className="text-gray-700 dark:text-white mb-4 text-[12px] font-thin ">
                Sign up for 7 days a free trial and explore our amazing
                features! We understand the importance of finding the perfect
                domain to elevate your online presence.
              </p>

              {user ? (
                <button
                  type="submit"
                  className="bg-blue-500 btn text-white p-2 rounded-md hover:bg-blue-600 focus:outline-none focus:ring focus:border-blue-300"
                >
                  Free Trial Application
                </button>
              ) : (
